...
def list_round1(list, n):
    new_list = []
    for i in range(len(list)):
        new_list.append(list[i].round(n))
    return new_list


def list_round(list, n):
    new_list = []
    for i in range(len(list)):
        new_list.append(round(list[i], n))
    return new_list

# 读取文件列表
file_folder_path = r'E:\PycharmProjects\regression\fig_in_article\非线性数据 -变换'
df_r = pd.DataFrame()
...
for i in range(len(df_seg_list)):
    ...
    X_data = df_seg_list[i][['yxs1', 'yxs2', 'yxpg', 'opi']].values
    ...
    plt.text(x_temp - 0.2, - 0.6, r'$%s$' % round(x_temp, 2), fontdict={'size': '8', 'color': 'm'})


y_pred_list = combine_list(y_pred_list)
df_sort['bxs2_eval'] = y_pred_list
MSE_ = np.sqrt(mean_squared_error(df_sort['bxs2'], df_sort['bxs2_eval']))
R_SCORE_ = r2_score(df_sort['bxs2'], df_sort['bxs2_eval'])
print(MSE_)
print(R_SCORE_)

plt.title('Region10_HeavyOil')
plt.xlabel('rock debris (mg/g)')
plt.ylabel('sidewall coring (mg/g)')

plt.ylim(0)

COEF_list_round = list_round1(COEF_list, 4)
intercept_round = list_round(intercept, 4)
FUNC = []
for i in range(len(COEF_list)):
    FUNC_ = 'bxs2 = %s + %s × yxs1 + %s × yxs2 + %s × Pg + %s × OPI >>>>>>>%s'%(intercept_round[i], COEF_list_round[i][0], COEF_list_round[i][1], COEF_list_round[i][2], COEF_list_round[i][3], i+1)
    FUNC_ = FUNC_.replace('+ -', '- ')
    FUNC.append(FUNC_)
